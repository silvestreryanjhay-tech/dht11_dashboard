...
# On hosts like Render, you can't drop a local file into the deployed app
# (and shouldn't commit the key to GitHub). Instead, paste the FULL
# contents of serviceAccountKey.json as an environment variable named
# FIREBASE_KEY_JSON in the Render dashboard (Environment tab). This is
# checked first; the local file is used as a fallback for local runs.
...
try:
    ...
    if FIREBASE_KEY_JSON:
        ...
        FIREBASE_READY = True
        print("[firebase] initialized using FIREBASE_KEY_JSON env var")
        ...
        history_ref = firebase_db.reference("history")
        FIREBASE_READY = True
        print(f"[firebase] initialized using key file at {FIREBASE_KEY_PATH}")
    else:
        print(f"[firebase] NOT initialized: no FIREBASE_KEY_JSON env var set, "
              f"and no key file found at {FIREBASE_KEY_PATH}. "
              f"Dashboard will show simulated/no data until this is fixed.")
...
# Start the background poller as soon as this module is imported, not just
# when run directly -- Render (and most hosts) run this via gunicorn/wsgi,
# which imports the module without executing the __main__ block below.
